	...
	return (
		<section data-section ref={ref} id="home" className="h-full flex flex-col justify-center items-center  ">
			...
			<div className="flex flex-col items-center mt-8 gap-4 text-center max-w-56 md:max-w-[600px]  2xl:max-w-[1000px]">
				<p className="text-3xl font-medium md:text-4xl lg:text-5xl 2xl:text-7xl">{data.home.greetings} <span className="text-brand-400">{data.home.name}</span></p>
				<p className="text-sm md:text-base lg:text-lg 2xl:text-xl">{data.home.description}</p>
				<Button onClick={() => window.open(data.home.linkedin)}>
					Connect with me
					<ArrowRight size={16}/>
				</Button>
			</div>
